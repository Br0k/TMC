      ...
      if (bcm2835_gpio_eds(RF_IRQ_PIN)) {
        ...
        if (rf95.available()) { 
          ...
          if (rf95.recv(buf, &len)) {
            ...
            int ret = AES_set_decrypt_key((unsigned char*) userkey, BLOCK_SIZE*8, &dec_key);
            printf("%d\n",ret );
            unsigned char dataDec[len];
            AES_cbc_encrypt(buf, dataDec, BLOCK_SIZE, &dec_key, IV, AES_DECRYPT);
            printf("Received encrypted Data: ");

            hex_print(buf, len);
            printf("After deciphering:\n");
            //printf("Packet[%02d] #%d => #%d %ddB: ", len, from, to, rssi);
            printbuffer((uint8_t*) dataDec, strlen((const char*)dataDec));
          } else {
